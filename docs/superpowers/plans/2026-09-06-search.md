...
and buildOutline (heading extraction that's fence-aware — # inside a
```` ``` ```` code block is not treated as a heading, per the
fence-aware text-processing lesson in docs/LESSONS.md from md-reader).
